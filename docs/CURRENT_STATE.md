...
- M4R 正文重寫：已完成 1,031 筆可用 lesson 的出版社研究導向完整正文；逐題外部考題核驗、教師／專家審查與遠端 Pages 發布驗收仍待另行進行。詳細範圍見 `docs/FULL_LESSON_SCOPE_AUDIT_2026-08-27.md`。
- M5R：前端已依 `full-lesson-v1` 優先呈現完整正文、摘要、離堂檢核、互動與出版社研究來源；已在本機以 A-7-3 實測載入與答題，遠端 Pages 部署驗收仍待全量內容品質抽查後進行。
