...
- M4：1,032/1,032 coverage rows 已建立 lesson 與題庫欄位；其中 116 rows／120 lessons 為 `content-reviewed`，916 rows／916 lessons 明確維持 `draft`。每單元至少 10 題，數學／自然具 3 步互動；仍不代表教師／學科專家審閱。
...
- M4 原創內容：目前 1,036 份 lesson、10,360 題 question，均標示 `origin=original` 並連至既有 KG ID；lesson 狀態為 120 `content-reviewed`、916 `draft`，仍不宣稱外部教師審閱完成。
...
- M4 題庫審查狀態：1,200 題為 `content-reviewed`、9,160 題仍為 `draft`；此欄位與 coverage row／lesson 的內部 QA 狀態分開計算，未宣稱教師審閱完成。
