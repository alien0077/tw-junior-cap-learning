...
4. 出版商公開資料或合法取得的版本資料：M3 用於版本 mapping；M4 另須逐單元建立版本研究摘要，再由 LLM 以原創文字融合，不得將出版社內容直接當成 lesson。

官方課綱 URL、發布日與定位集中記錄在 docs/SOURCES.md 及各 curriculum／knowledge JSON 的 sources 欄位。

## Lesson 編寫流程

1. 選定一個或多個 KG ID，寫出可觀察的學習目標與單元邊界。
2. 逐一讀取可取得的各版本資料，建立 `publisherResearch` 的內容研究，不得只填章節名稱或泛用句子。
3. 比較各版本的概念順序、用語、表徵、例型、活動／史料、常見錯誤與評量方向，標出共同核心與差異。
4. 由 LLM 以官方課綱、KG 與版本研究融合撰寫完整原創正文：摘要、核心概念、推理／判讀步驟、具體自編例子、錯誤診斷、遷移任務、自我檢核與限制。
5. 數理科補足計算／模型／實驗證據與變式；社會科補足時間線、空間尺度、資料證據、因果限制、不同觀點與公民判斷。
6. 檢查沒有依賴出版社課文、例句、圖片、習題、教師手冊或近似改寫；研究未完成時維持 `draft`。
7. 經第一輪 AI 自審與 Terra 第二輪 AI 複核後，才可標記 `reviewStatus=content-reviewed`，並確認所有 KG endpoint 存在。
