...
- 直接請求補充：目前本機憑證驗證無法建立該站 TLS 信任鏈（curl error 60）；未以跳過憑證驗證方式下載，避免把未驗證內容寫入 repo。
- 結案：M3 改以國教院冊別資料加上公開國中課程計畫／教育平台的可追溯章節證據驗證；五科六冊均有證據，維持校方來源 `medium` confidence。詳見 `docs/M3_M4_COMPLETION_EVIDENCE_2026-08-27.md`。
...
- 結案：翰林非國文四科各六冊已由出版社公開索引或公開國中課程計畫提供可回溯章節／單元證據；未把校方證據誤標為出版社目次。詳見 `docs/M3_M4_COMPLETION_EVIDENCE_2026-08-27.md`。
...
## B-004：M5 MVP 部署版本待發佈

- 狀態：已解決（2026-08-27）。
- 影響：先前 Pages 上的 `data-manifest.json` 是範例載入；本次 M5 改為每次部署由固定 Git commit 建置完整 `data-index.json`，未推送前遠端不會反映新版本。
- 已完成：靜態前端、固定 revision 資料索引、五科篩選、搜尋、分頁、教材與 10 題的延遲載入、錯誤狀態及資料索引 validator；本機瀏覽器已實測數學篩選、`A-7-1` 搜尋與完整題組載入，主控台無 error。
- 遠端結果：GitHub Pages workflow `33047828285` 與 data validation workflow `33047828275` 均成功；公開首頁已驗證顯示固定版本 `14fd8a81422b`、1,031 份教材與 10,310 題。
- 後續：每次資料更新仍以新的固定 commit 重做同一組建置與 smoke test。
...
- 最新權威快照（2026-08-26）：以目前檔案重新計算為 1,210 題 `content-reviewed`、9,150 題 `draft`；上方 9,160／9,400 為歷史批次數字，不能作為目前狀態。
...
- 最新自動稽核：通用模板 lesson 為國文 63、英文 131、數學 47、自然 315、社會 359，共 915 份；其中重複題幹題數分別為 280、370、0、2,780、3,070。這些數字只能排序 QA 優先順序，不能取代學科內容審核。
