...
by_subject = collections.defaultdict(lambda: {"lessons": 0, "questions": 0, "duplicate_prompts": 0, "generic_options": 0, "interactive_missing": 0, "cap_source": 0, "curriculum_source": 0})
...
lines = ["# M4 Draft QA 稽核報告", "", "此報告只用於排序內容 QA，不會將 draft 升級為 content-reviewed。重複題幹以同科同文字計算，可能反映相同課綱概念，仍需人工核對。", "", "| 科目 | draft lessons | draft questions | 重複題幹題數 | 模板選項題數 | 缺互動 lesson | CAP 來源題數 | 課綱來源題數 |", "|---|---:|---:|---:|---:|---:|---:|---:|"]
for subject in ("chinese", "english", "math", "science", "social"):
    row = by_subject[subject]
    lines.append(f"| {subject} | {row['lessons']} | {row['questions']} | {row['duplicate_prompts']} | {row['generic_options']} | {row['interactive_missing']} | {row['cap_source']} | {row['curriculum_source']} |")
lines += ["", "## 來源與狀態說明", "", "- 所有 draft 題目仍維持 `reviewStatus=draft`；來源 URL 僅表示課綱／會考範圍定位，不表示題目已通過學科審閱。", "- CAP 來源為教育部國中教育會考官方範圍頁；其餘為國家教育研究院課程綱要頁。", "- 目前沒有任何 draft lesson 因此報告而自動升級狀態。"]
