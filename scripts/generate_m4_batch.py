...
def main()->int:
 ...
 for r in rows:
  ...
  for i in range(1,11):
   q={"id":f"question-{a.subject}-{key[len(a.subject)+1:] if key.startswith(a.subject+'-') else key}-{i}","subject":a.subject,"type":"single-choice","prompt":f"【草稿】學習「{r['title']}」時，哪一項最符合課綱學習方向？","options":[{"id":"A","text":f"理解「{r['title']}」並能應用"},{"id":"B","text":"只記頁碼"},{"id":"C","text":"只背選項順序"},{"id":"D","text":"跳過學習目標"}],"knowledgeIds":[kg],"difficulty":"easy" if i<=3 else "medium","answer":{"value":"A","explanation":f"本題檢查「{r['title']}」的核心理解與應用。"},"provenance":{"origin":"original","license":"All rights reserved","authoringNote":"Batch-generated draft; subject QA required."},"reviewStatus":"draft","updatedAt":"2026-08-26","lessonId":lid};(qd/f"{q['id']}.json").write_text(json.dumps(q,ensure_ascii=False,indent=2)+"\n")
