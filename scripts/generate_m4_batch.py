...
def main()->int:
 ...
 m["summary"]["questionsCovered"]=sum(r.get("questionCount",0)>=10 and r.get("reviewStatus") in {"content-reviewed","published"} for r in m["rows"]);mp.write_text(json.dumps(m,ensure_ascii=False,indent=2)+"\n");print(f"materialized {len(rows)} {a.subject} draft lessons and {len(rows)*10} draft questions");return 0
